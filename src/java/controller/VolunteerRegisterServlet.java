...
//      int userid = (int) request.getAttribute("Userid");
        ...
        session.setAttribute("acc", d.selectUserById(8));
        User user = (User) session.getAttribute("acc");
//        int user_id = user.getUserId();
//        Volunteer vl = vlD.VolunteerByUserId(7);
//        int vl_id = vl.getVolunteerId();
        String description = request.getParameter("description");
        String description_skill = request.getParameter("descriptionskill");
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        int user_id = user.getUserId();
        Volunteer vl = vlD.VolunteerByUserId(user_id);
        int vl_id = vl.getVolunteerId();
        Date created_at = new Date();
        //            Date crat = new Date();
        String ct = dateFormat.format(created_at);
        VolunteerCV newcv = new VolunteerCV(0, vl_id, description, description_skill, ct);
