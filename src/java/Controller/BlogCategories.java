 ...
 * @author admin
 */
@WebServlet(name = "BlogCategories", urlPatterns = {"/BlogCategories"})
public class BlogCategories extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       response.setContentType("text/html;charset=UTF-8");
        ...
        int count = dao.getTotalNewsbyCID(cateID);
        int endPage = count / 5;
        if (count % 5 != 0) {
            endPage++;
        }
        ...
        if (cateID != null && !cateID.isEmpty()) {
            List<News> list = dao.getNewsByCID(cateID, index);
