...
public class StatusBlog extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int count = 0;
        int endPage = 0;
        List<News> status = null;
        if ("On".equals(sortBy)) {
             count = dao.getTotalNewswithOn();
             endPage = count / 5;
            if (count % 5 != 0) {
                endPage++;
            }
            status = dao.getAllNewsbyAdminwithOn(index);

        } else if ("Off".equals(sortBy)) {
             count = dao.getTotalNewswithOff();
             endPage = count / 5;
            if (count % 5 != 0) {
                endPage++;
            }
            status = dao.getAllNewsbyAdminwithOff(index);
        }
        List<NewsCategories> listC = dao.getAllCategory();

        request.setAttribute("listP", status);
        request.setAttribute("listCC", listC);
        request.setAttribute("endP", endPage);
        request.setAttribute("tag", index);
        request.setAttribute("totalPage", count);
        request.setAttribute("by", sortBy);
