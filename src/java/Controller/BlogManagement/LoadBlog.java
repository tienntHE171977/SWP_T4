...
public class LoadBlog extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String categoryID = dao.getCategoryIDByNewsID(postID);

        News b = dao.getNewsByID(postID);
        List<NewsCategories> listC = dao.getAllCategory();
        request.setAttribute("categoryID", categoryID);
        request.setAttribute("detail", b);
        request.setAttribute("listCC", listC);
        request.getRequestDispatcher("Dashboard/EditBlog.jsp").forward(request, response);
    }
