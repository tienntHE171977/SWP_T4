        ...
        try {
            ...
            if (user != null) {
                if (orgDAO.getOByUserId(user.getUserId()) != null) {
                    if (!orgDAO.getOByUserId(user.getUserId()).getOrganizationStatus().equals("inactive")) {
                        throw new Exception();
                    }
                }
...
// Lưu file vào thư mục /uploads thay vì build
        String uploadPath = "D:\\SWP391\\Swp_2\\Swp\\web\\images"; // Đường dẫn đến thư mục images
...
//        OrganizationDAO orgDAO = new OrganizationDAO();
        if (orgDAO.getOByUserId(user.getUserId()) != null) {
            Organization org = new Organization(orgDAO.getOByUserId(user.getUserId()).getOrganizationId(), name, website, des, imageUrl, Phone, Email, user, 3, crat, establishmentTime, "pending");
            orgDAO.update(org);
        } else {
            Organization org = new Organization(0, name, website, des, imageUrl, Phone, Email, user, 3, crat, establishmentTime, "pending");
            try {
                orgDAO.insert(org);
            } catch (Exception ex) {
                Logger.getLogger(RegisOrganizationServlet.class.getName()).log(Level.SEVERE, null, ex);
